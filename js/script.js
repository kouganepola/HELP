...
const createMainFlowChart = (data) => {
  ...
  const subgraph_data = [{ graph: 'Graph1', x: 16000, y: 22000 },
    { graph: 'Graph2', x: 18000, y: 23000 }]
  ...
  subgraphs.on("click", (d,i) => {
    ...
    d3.text("../html/awareness_graph.html").then((data) => { 
      
      overlay.append('div')
        .attr('viewBox', [0, 0, width, height])
        .attr('id', 'model')
        .attr("style", "max-width: 90%; max-height: 90%; font: 10px sans-serif; background-color:#dddddd59; outline: thin solid #E0E0E0; ")
        .attr("preserveAspectRatio", "xMinYMin meet")
        .html(data);
